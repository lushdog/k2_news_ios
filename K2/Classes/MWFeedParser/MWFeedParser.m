		...
		switch (feedType) {
			case FeedTypeRSS: {
				...
				if (!processed) {
					...
					else if ([currentPath isEqualToString:@"/rss/channel/item/content:encoded"]) { if (currentText.length > 0) item.content = currentText; processed = YES; item.img = [self scanForImage:currentText];}
			...
			case FeedTypeRSS1: {
				...
				if (!processed) {
					...
					else if ([currentPath isEqualToString:@"/rdf:RDF/item/content:encoded"])  { if (currentText.length > 0) item.content = currentText; processed = YES; item.img = [self scanForImage:currentText];} 
			...
			case FeedTypeAtom: {
				...
				if (!processed) {
					...
					else if ([currentPath isEqualToString:@"/feed/entry/content"])  { if (currentText.length > 0) item.content = currentText; processed = YES; item.img = [self scanForImage:currentText];} 
